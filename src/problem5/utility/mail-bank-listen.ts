...
import { IUserService } from '../src/user/i.user.service';
...
export async function emailAndPasswordImap(
  { user, password },
    ...
    imap.on('mail', function (numNewMsgs) {
      ...
      openInbox(function (err, box) {
        ...
        imap.search(['UNSEEN', ['FROM', senderEmail]], function (err, results) {
          if (err) throw err;
          console.log(
            `Found ${results.length} unseen message(s) from ${senderEmail}`,
          );
          if (results.length > 0) {
              ...
              msg.on('body', function (stream, info) {
                simpleParser(stream, async (err, mail) => {
                  ...
                  if ( SyntaxTransfer && syntaxCode &&
                    ...
                    const transactionDto = await service.autoCreateTransaction(
                      secretCodeClientSend,
                      money,
                      DEPOSIT.VP,
                    );
                  }
